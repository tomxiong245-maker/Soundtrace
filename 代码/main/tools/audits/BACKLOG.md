...
**当前状态**：tools.json 共 **57 项**（2026-08-19 加 6 项 cut-verify）· audit 覆盖率 **8/57**（ffmpeg + extract_gold_cut_features + p0_transcribe_mvp + detect_self_correction + detect_self_correction_wordlevel + mfa_align_and_extract_boundaries + apply_autocut_gate + write_delivery_report + **cut-verify v1 全套** 6 项 · 单一 audit md）· runtime_dependencies 覆盖率 **4/57**

**任务**：为剩下 tool 补 audit md（照抄 `ffmpeg-homebrew-9.0.1.md` 四段结构）。分两批优先级：
...
## ✅ 已有 audit（13 项 · 8/57 · 含 cut-verify skill 全套）
...
| **cut-verify skill 全套 6 项**（`check_hallucination` · `check_silence_location` · `check_rhythm_gap` · `route_crossfade_strategy` · `verify_cut_plan` · `expand_to_asr_word_boundary`）| ✅ | ✅ | **2026-08-19 新增** · `skills/cut-verify/audits/cut-verify-v1.md` 覆盖 6 脚本 · 剪口干净度 4 项 check + ASR-word 边界扩展经验落地. 依赖 pydub / faster-whisper.word.probability / cut_parameters.json |
...
> 2026-08-19 · 移除 `feedback_first_retrieval` 与 `user_feedback_analyzer` 两项 LEGACY SHIM：已并入 `feedback_engine`；对应 `.py` 与 `tools.json` 条目删除；skills/feedback-engine/SKILL.md 的 legacy alias 引用作为历史文档保留。
