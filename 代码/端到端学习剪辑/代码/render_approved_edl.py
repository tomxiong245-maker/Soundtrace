...
def main() -> int:
    ...
    if any(not path.is_absolute() for _, path in tracks) or not edl_path.is_absolute() or not output_dir.is_absolute():
        parser.error("all paths must be absolute")
    if len(tracks) < 2:
        parser.error("at least two aligned tracks are required")
    ...
    targets = [*stem_paths, mix_path, manifest_path]
