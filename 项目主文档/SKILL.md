...
## 0. 从用户视角：3 问 → 7 skill 分工

> **配套阅读**：[`剪辑核心-3问与20参数.md`](剪辑核心-3问与20参数.md)（顶层理解入口 · 3 分钟看懂）

整套系统只回答一件事 —— **一个候选（比如"呃"字）到底剪不剪 · 剪的话在哪切 · 切完怎么拼**。

7 个 active skill 从"回答哪一问"角度的分工：

| 问题 | 主要哪些 skill 服务 | 何时被调 |
|---|---|---|
| **要不要剪** | `candidate-generation-and-gate`（候选 + 7 门 gate）· `feedback-engine`（查 never_cut）· `cut-verify` Check 1（幻觉） | pipeline Stage 3-4 |
| **在哪切** | `candidate-generation-and-gate`（MFA + neighbor + onset 边界精修）· `cut-verify` ASR-word 扩展 | Stage 3.5 + cut-verify 侧车 |
| **怎么拼** | `cut-verify` Check 2/3/4（静音位置 + 节奏 + 路由）· `audition-and-delivery`（automix apply-EDL + xfade） | cut-verify 侧车 + Stage 6 |

横切支持：
- `feedback-engine`（L1）· 每次决策前后都查 · 服务 3 问全过程
- `learning-and-experience`（L3）· 消费 3 问的历史决策
- `governance-and-tool-registry`（L4）· 保证 3 问所依赖的 tool 都登记
- `episode-triage-and-plan`（L0）· 保证进入 3 问决策前 · speaker_map / plan.json 已就位

**参数 20 个 · 分 4 类**（`knowledge/cut_parameters.json` 权威源）：
- A（5 个）· 决定"要不要剪"
- B（7 个）· 决定"在哪切"
- C（5 个）· 决定"怎么拼"
- D（3 个）· 决定"节奏 OK 吗"

**历史所有 bug 的根源**（一件事）：见 [`根因-文档法与代码法.md`](根因-文档法与代码法.md)。
