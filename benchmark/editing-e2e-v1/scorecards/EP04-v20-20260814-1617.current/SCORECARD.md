...
- run: `EP04-v20-20260814-1617` / `EP04`
...
- 全部候选：5（5.500 条/节目小时）；审核包：5（5.500 条/节目小时）。
- 候选类别：`{"filler_hesitation": 1, "immediate_repetition": 4}`；风险：`{"low": 5}`。
- 审核预算 / 剩余：`20` / `15`；安全阻断：`25`。
...
- 状态：`PENDING_HUMAN_REVIEW`；正式决定：0 / 5。
