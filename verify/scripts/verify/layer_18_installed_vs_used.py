...
def _find_project_root() -> pathlib.Path:
    """从脚本位置往上找，直到看到 CLAUDE.md（或 项目主文档/CLAUDE.md）· 自适应 live 布局
    (scripts/verify/*.py) 和 交付 布局 (verify/scripts/verify/*.py)。"""
...
ROOT = pathlib.Path(os.environ.get("PROJECT_ROOT") or _find_project_root())
...
# ---- 项目实际扫描的目录（源代码所在处）
SCAN_DIRS = [
    "代码/main/orchestrator",
    "代码/main/tools",
    "代码/稳定生产/challengers",
    "verify/scripts",
    # legacy path fallback (原项目布局)
    "main/orchestrator",
    "main/tools",
    "稳定生产/challengers",
    "端到端学习剪辑/代码",
...
def scan_source_imports() -> set[str]:
    ...
    # 排除路径：venv / site-packages / __pycache__ / 归档 / 交付历史包 / 第三方 clone
    EXCLUDE_PARTS = {"venv", "venv-mvp", ".venv", "site-packages", "__pycache__",
                     "归档", "交付", "node_modules", ".git", "dist", "build"}
    for d in SCAN_DIRS:
        for py in (ROOT / d).rglob("*.py"):
            # 跳过 venv / site-packages / __pycache__ 等第三方目录
...
def get_declared_deps() -> set[str]:
    ...
    d = json.loads((ROOT / "代码/main/tools/tools.json" if (ROOT / "代码/main/tools/tools.json").exists() else ROOT / "main/tools/tools.json").read_text())
    declared = set()
...
def main():
    ...
    if undeclared_installed:
        ...
        sys.exit(1)
    if unused_installed:
        # 装了但没用大多是 transitive 依赖 · 保持 WARN 级 · 不 block verify.sh
        print("[WARN] 装了但没用的包 → 违反 CLAUDE.md §15 · 建议清理或补 import · 但不 block（transitive 依赖常见）")
        sys.exit(0)
